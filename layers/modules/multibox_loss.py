...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        pos = conf_t > 0  #pos 找出模型 对 priors分类后 不为背景 的prior  conf_t shape: torch.size(batch_size, num_priors)
        num_pos = pos.sum(dim=1, keepdim=True) # conf_t 中不为背景的个数

        # Localization Loss (Smooth L1)
        # Shape: [batch,num_priors,4]
        pos_idx = pos.unsqueeze(pos.dim()).expand_as(loc_data)#loc_data shape: torch.size(batch_size,num_priors,4)
